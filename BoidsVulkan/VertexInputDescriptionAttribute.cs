...
{ 
    (VertexInputAttributeDescription[], VertexInputBindingDescription) CreateVertexInputDescription(int binding, VertexInputRate inputRate = VertexInputRate.Vertex)
    {
        var result = new List<VertexInputAttributeDescription>();


        foreach (var (property, attribute) in typeof(TSelf).GetFields()
                                ...
                                .Where(z => z.Item2 != null))
        {
            result.Add(new VertexInputAttributeDescription()
            {
                ...
                Offset = (uint)Marshal.OffsetOf<TSelf>(property.Name)
            });
        }
